...
#BiRNNmodel = DeepRecurrent(['GRU', 'Simple'], [_INPUT_UNITS, _HIDDEN_UNITS], _INPUT_UNITS, _OUTPUT_UNITS, _LOOKBACK_SAMPLES)
#RNNmodel = DeepBidirectionalRecurrent(_INPUT_UNITS, _HIDDEN_UNITS, _OUTPUT_UNITS, _LOOKBACK_SAMPLES)
...
checkpointerBiGRU = ModelCheckpoint(filepath="./model/best_validation_BiGRU_model_weights.h5", verbose=1, save_best_only=True)
checkpointerBiRNN = ModelCheckpoint(filepath="./model/best_validation_BiRNN_model_weights.h5", verbose=1, save_best_only=True)
...
BiRNNmodel.fit([X_Train, X_Train], Y_Train, batch_size=1000, nb_epoch=_NEPOCH, shuffle = True, validation_data=([X_Validation, X_Validation], Y_Validation), callbacks=[checkpointerBiRNN, hist, stopEarly])
BiGRUmodel.fit([X_Train, X_Train], Y_Train, batch_size=1000, nb_epoch=_NEPOCH, shuffle = True, validation_data=([X_Validation, X_Validation], Y_Validation), callbacks=[checkpointerBiGRU, hist, stopEarly])

### Training Completed ###
print("Training the Neural Network : Completed.")

#------------------------------------------ Save RNN Model --------------------------------------------#
saveModel(BiGRUmodel, 'BiGRU')
saveModel(BiRNNmodel, 'BiRNN')
