...
Output => Model
'''
def DeepBidirectionalRecurrent(_TYPE, _INPUT_UNITS, _HIDDEN_UNITS, _OUTPUT_UNITS, _LOOKBACK_SAMPLES):
    if _TYPE == 'RNN':
        ...
        left, right = fork_left_1, fork_right_1

        ############-------------- Third Layer : Bidirectional--------------############
        left.add(SimpleRNN(2*_HIDDEN_UNITS, return_sequences = False, go_backwards = False, init = 'lecun_uniform', activation='tanh'))
        right.add(SimpleRNN(2*_HIDDEN_UNITS, return_sequences = False, go_backwards = True, init = 'lecun_uniform', activation='tanh'))

        model = Sequential()
        model.add(Merge([left, right], mode='concat'))
        model.add(TimeDistributedDense(_OUTPUT_UNITS, activation = 'relu'))
    ...
    if _TYPE == 'GRU':
        ...
        ############-------------- Third Layer : Bidirectional--------------############
        left.add(GRU(2*_HIDDEN_UNITS, return_sequences = False, go_backwards = False, init = 'lecun_uniform', activation='tanh'))
        right.add(GRU(2*_HIDDEN_UNITS, return_sequences = False, go_backwards = True, init = 'lecun_uniform', activation='tanh'))
